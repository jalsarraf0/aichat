...
def _batch_create(jobs: list[dict]) -> dict:
    r = httpx.post(f"{JOBS_URL}/batch", json={"jobs": jobs}, timeout=_TIMEOUT)
    r.raise_for_status()
    return r.json()
...
class TestBatchCreate:
    def test_batch_returns_ids(self):
        ...
        assert "ids" in result or "jobs" in result, f"Unexpected batch result: {result}"
        ids = result.get("ids") or [j["id"] for j in result.get("jobs", [])]
        assert len(ids) == 2, f"Expected 2 job IDs, got {ids}"

    def test_batch_empty_list_rejected(self):
        r = httpx.post(f"{JOBS_URL}/batch", json={"jobs": []}, timeout=_TIMEOUT)
        assert r.status_code in (400, 422), f"Expected 4xx for empty jobs list, got {r.status_code}"
...
class TestJobLifecycleE2E:
    """Full job lifecycle: create → poll → terminal state."""

    def test_simple_job_completes(self):
        """A real memory_store job should reach succeeded or failed within 15s."""
        job = _create_job("memory_store", {"key": f"e2e_{_uid()}", "value": "lifecycle"})
        final = _wait_for_terminal(job["id"], max_wait=15)
    ...
    def test_job_with_bad_tool_fails_gracefully(self):
        """A job for a non-existent tool must fail, not hang."""
        job = _create_job(f"no_such_tool_{_uid()}")
        final = _wait_for_terminal(job["id"], max_wait=10)
    ...
    def test_batch_jobs_all_terminal(self):
        ...
        ids = batch.get("ids") or [j["id"] for j in batch.get("jobs", [])]
        for jid in ids:
            final = _wait_for_terminal(jid, max_wait=20)
