...
class TestLoggingSystem:
    ...
    def test_http_mcp_get_errors_schema_present(self):
        ...
        # get_errors is now data.errors action in the mega-tool architecture
        assert "data" in tool_names, f"data mega-tool missing from HTTP MCP _TOOLS: {tool_names}"
        data_tool = next(t for t in mod._TOOLS if t["name"] == "data")
        data_actions = data_tool["inputSchema"]["properties"]["action"]["enum"]
        assert "errors" in data_actions, f"errors action missing from data mega-tool: {data_actions}"
