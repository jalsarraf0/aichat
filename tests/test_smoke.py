...
def test_mcp_health_includes_tools() -> None:
    ...
    assert body["tools"] >= 50, f"Expected ≥50 tools, got {body['tools']}"
...
def test_mcp_tools_list_via_jsonrpc() -> None:
    ...
    assert len(tools) >= 50, f"Expected ≥50 tools from MCP, got {len(tools)}"
    tool_names = {t["name"] for t in tools}
    for expected in ("screenshot", "web_search", "memory_store", "get_errors",
                     "think", "realtime", "wikipedia", "jupyter_exec"):
