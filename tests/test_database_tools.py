...
class TestDbCacheGetE2E:
    ...
    def test_valid_get_sends_url_param(self):
        ...
        # Bug fix: data service /cache/get uses 'key=', not 'url=' param
        assert params.get("key") == "https://example.com/page"
...
class TestDbToolsSmoke:
    ...
    def test_db_cache_store_and_get(self):
        ...
        assert texts
        parsed = json.loads(texts[0])
        assert parsed.get("stored") or parsed.get("status") == "cached", f"Unexpected: {parsed}"
    ...
    def test_db_store_article_url_only(self):
        ...
        json.loads(texts[0]) if texts[0].startswith("{") else {}
    ...
    def test_db_search_returns_articles(self):
        ...
        assert texts
        parsed = json.loads(texts[0])
        assert "articles" in parsed or "results" in parsed or "error" in parsed, f"Unexpected response: {parsed}"
