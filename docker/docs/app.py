...
# Maximum ingest size: 50 MB for base64 payloads, 25 MB for remote fetches
_MAX_B64_BYTES = 50 * 1024 * 1024
_MAX_FETCH_BYTES = 25 * 1024 * 1024

# SSRF guard: deny fetches to RFC-1918 / loopback / link-local ranges
import ipaddress
_BLOCKED_NETS = [
...
def _check_url_allowed(url: str) -> None:
    ...
    try:
        ...
        for net in _BLOCKED_NETS:
            if ip in net:
                raise HTTPException(status_code=403, detail="URL resolves to a blocked internal address")
    except HTTPException:
        raise
    except Exception:
        pass  # DNS failure handled by the fetch itself
...
def ingest_b64(payload: dict) -> dict:
    ...
    if len(b64) > _MAX_B64_BYTES * 4 // 3 + 4:
        raise HTTPException(status_code=413, detail=f"Payload too large (max {_MAX_B64_BYTES // 1024 // 1024} MB)")
    try:
        data = base64.b64decode(b64)
    except Exception as exc:
        raise HTTPException(status_code=422, detail="Invalid base64 data") from exc
    if len(data) > _MAX_B64_BYTES:
        raise HTTPException(status_code=413, detail=f"Decoded document too large (max {_MAX_B64_BYTES // 1024 // 1024} MB)")
...
async def ingest_url(payload: dict) -> dict:
    ...
    if len(data) > _MAX_FETCH_BYTES:
        raise HTTPException(status_code=413, detail=f"Remote document too large (max {_MAX_FETCH_BYTES // 1024 // 1024} MB)")
