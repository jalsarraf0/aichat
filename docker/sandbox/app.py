"""aichat-sandbox: dynamic custom tool host (sandboxed).
...
log = logging.getLogger("aichat-sandbox")

# ---------------------------------------------------------------------------
# Error reporting
# ---------------------------------------------------------------------------

_DATABASE_URL = os.environ.get("DATABASE_URL", "http://aichat-data:8091")
_SERVICE_NAME = "aichat-sandbox"
...
app = FastAPI(title="aichat-sandbox")
...
# Auto-generated by aichat-sandbox
# GPU packages available when /dev/dri is mounted: numpy, cv2 (opencv-python-headless)
import asyncio
import json
import math
import os
...
# ---------------------------------------------------------------------------
# ToolkitGpuRuntime — GPU detection for the toolkit sandbox
# ---------------------------------------------------------------------------

class ToolkitGpuRuntime:
    """Detects GPU availability inside aichat-sandbox and reports which
    GPU-accelerated packages are importable to custom tool authors.

    When /dev/dri is mounted (via docker-compose devices) and INTEL_GPU=1
    is set, custom tools can import numpy and cv2 with OpenCL acceleration.
    """
    ...
    def available(cls) -> bool:
        """True if any GPU access is detected (DRI device or INTEL_GPU env)."""
        return cls._has_dri() or os.environ.get("INTEL_GPU", "") == "1"

    @classmethod
    def packages(cls) -> list[str]:
        """Return importable GPU-capable packages in this container."""
        candidates = ["numpy", "cv2", "scipy", "torch", "cupy"]
        result: list[str] = []
        import importlib
...
async def health() -> dict:
    return {"status": "ok", "tools_dir": str(TOOLS_DIR), "gpu": ToolkitGpuRuntime.info()}
