...
class LLMClient:
    ...
    async def list_loaded_models(self) -> list[str]:
        ...
        Uses the LM Studio-specific ``/api/v0/models`` endpoint which includes
        a ``state`` field (``"loaded"`` or ``"not-loaded"``).
        Returns an empty list on any failure (fail-open).
        """
        try:
            response = await self._request("GET", "/api/v0/models")
            data = response.json().get("data", [])
            return [
                m["id"]
                for m in data
    ...
    async def ensure_model_or_busy(self, model: str) -> None:
        """Verify *model* can be served without evicting another user's model.

        - Model already loaded → OK.
        - Model not loaded but slots available → OK (LM Studio will load it).
        - Model not loaded and all slots occupied → raise ``ModelBusyError``.
        """
        loaded = await self.list_loaded_models()
        if not loaded:
            return  # can't determine state — fail-open
        if model in loaded:
            return
        if len(loaded) >= _LM_STUDIO_MAX_LOADED:
            raise ModelBusyError(
                f"All {_LM_STUDIO_MAX_LOADED} model slots are in use "
                f"({', '.join(loaded)}). Try again later or switch to a loaded model."
            )
