...
from .client import LLMClient, LLMClientError, ModelBusyError
...
class AIChatApp(App):
    ...
    async def run_llm_turn(self) -> None:
        ...
        # Guard: refuse to evict another user's model when LM Studio is full.
        try:
            await self.client.ensure_model_or_busy(self.state.model)
        except ModelBusyError as exc:
            self._write_transcript("Assistant", f"\u26a0\ufe0f {exc}")
    ...
    async def _execute_tool_call(self, name: str, args: dict[str, object]) -> str:
        ...
        if name == "recognize_face":
            image_url = str(args["image_url"]).strip() if args.get("image_url") else None
            image_b64 = str(args["image_base64"]).strip() if args.get("image_base64") else None
            image_file = str(args["image_file"]).strip() if args.get("image_file") else None
            if not image_url and not image_b64 and not image_file:
                return "recognize_face: provide image_url, image_base64, or image_file"
            try:
                limit = int(args.get("limit", 5))
                min_conf = float(args.get("min_confidence", 0.7))
            except (TypeError, ValueError):
                limit, min_conf = 5, 0.7
            payload = await self.tools.run_recognize_face(
                image_url, image_b64, image_file, limit, min_conf,
                self.state.approval, self._confirm_tool,
            )
            if "error" in payload:
                return f"recognize_face failed: {payload['error']}"
            faces = payload.get("faces", [])
            if not faces:
                return "No faces recognized in the image."
            lines = []
            for i, face in enumerate(faces, 1):
                matches = face.get("matches", [])
                if matches:
                    top = matches[0]
                    lines.append(f"Face {i}: {top['subject']} (similarity {top['similarity']:.0%})")
                else:
                    lines.append(f"Face {i}: unknown")
            return f"Recognized {len(faces)} face(s):\n" + "\n".join(lines)
        if name == "verify_face":
            ia_url = str(args["image_a_url"]).strip() if args.get("image_a_url") else None
            ib_url = str(args["image_b_url"]).strip() if args.get("image_b_url") else None
            ia_b64 = str(args["image_a_base64"]).strip() if args.get("image_a_base64") else None
            ib_b64 = str(args["image_b_base64"]).strip() if args.get("image_b_base64") else None
            if not (ia_url or ia_b64) or not (ib_url or ib_b64):
                return "verify_face: provide image_a_url/image_a_base64 and image_b_url/image_b_base64"
            payload = await self.tools.run_verify_face(
                ia_url, ib_url, ia_b64, ib_b64,
                self.state.approval, self._confirm_tool,
            )
            if "error" in payload:
                return f"verify_face failed: {payload['error']}"
            verified = payload.get("verified", False)
            similarity = payload.get("similarity", 0.0)
            verdict = "MATCH" if verified else "NO MATCH"
            return f"Face verification: **{verdict}** (similarity {similarity:.0%})"
        if name == "detect_faces":
            image_url = str(args["image_url"]).strip() if args.get("image_url") else None
            image_b64 = str(args["image_base64"]).strip() if args.get("image_base64") else None
            image_file = str(args["image_file"]).strip() if args.get("image_file") else None
            if not image_url and not image_b64 and not image_file:
                return "detect_faces: provide image_url, image_base64, or image_file"
            try:
                min_conf = float(args.get("min_confidence", 0.8))
            except (TypeError, ValueError):
                min_conf = 0.8
            return_lm = bool(args.get("return_landmarks", False))
            payload = await self.tools.run_detect_faces(
                image_url, image_b64, image_file, min_conf, return_lm,
                self.state.approval, self._confirm_tool,
            )
            if "error" in payload:
                return f"detect_faces failed: {payload['error']}"
            ...
            return f"Detected {count} face(s):\n" + "\n".join(summaries)
        if name == "enroll_face":
            subject = str(args.get("subject_name", "")).strip()
            if not subject:
                return "enroll_face: 'subject_name' is required"
            image_url = str(args["image_url"]).strip() if args.get("image_url") else None
            image_b64 = str(args["image_base64"]).strip() if args.get("image_base64") else None
            image_file = str(args["image_file"]).strip() if args.get("image_file") else None
            if not image_url and not image_b64 and not image_file:
                return "enroll_face: provide image_url, image_base64, or image_file"
            payload = await self.tools.run_enroll_face(
                subject, image_url, image_b64, image_file,
                self.state.approval, self._confirm_tool,
            )
            if "error" in payload:
                return f"enroll_face failed: {payload['error']}"
            return f"Enrolled face for '{payload.get('subject', subject)}' (image_id: {payload.get('image_id', '?')})"
        if name == "list_face_subjects":
            payload = await self.tools.run_list_face_subjects(self.state.approval, self._confirm_tool)
            if "error" in payload:
                return f"list_face_subjects failed: {payload['error']}"
            subjects = payload.get("subjects", [])
            if not subjects:
                return "No face subjects enrolled."
            return f"Enrolled subjects ({len(subjects)}):\n" + "\n".join(f"  \u2022 {s}" for s in subjects)
        if name == "delete_face_subject":
            subject = str(args.get("subject_name", "")).strip()
            if not subject:
                return "delete_face_subject: 'subject_name' is required"
            payload = await self.tools.run_delete_face_subject(
                subject, self.state.approval, self._confirm_tool,
            )
            if "error" in payload:
                return f"delete_face_subject failed: {payload['error']}"
            deleted = payload.get("deleted", False)
            return f"Subject '{subject}' {'deleted' if deleted else 'not found'}."
        if name == "detect_objects":
            image_url = str(args["image_url"]).strip() if args.get("image_url") else None
            image_b64 = str(args["image_base64"]).strip() if args.get("image_base64") else None
            image_file = str(args["image_file"]).strip() if args.get("image_file") else None
            if not image_url and not image_b64 and not image_file:
                return "detect_objects: provide image_url, image_base64, or image_file"
            try:
                conf = float(args.get("confidence_threshold", 0.25))
            except (TypeError, ValueError):
                conf = 0.25
            payload = await self.tools.run_detect_objects(
                image_url, image_b64, image_file, conf,
                self.state.approval, self._confirm_tool,
            )
            if "error" in payload:
                return f"detect_objects failed: {payload['error']}"
            ...
            summary = ", ".join(f"{v}\u00d7 {k}" for k, v in label_counts.most_common(10))
            return f"Detected {count} object(s): {summary}"
        if name == "classify_image":
            image_url = str(args["image_url"]).strip() if args.get("image_url") else None
            image_b64 = str(args["image_base64"]).strip() if args.get("image_base64") else None
            image_file = str(args["image_file"]).strip() if args.get("image_file") else None
            if not image_url and not image_b64 and not image_file:
                return "classify_image: provide image_url, image_base64, or image_file"
            try:
                top_k = int(args.get("top_k", 5))
            except (TypeError, ValueError):
                top_k = 5
            payload = await self.tools.run_classify_image(
                image_url, image_b64, image_file, top_k,
                self.state.approval, self._confirm_tool,
            )
            if "error" in payload:
                return f"classify_image failed: {payload['error']}"
            labels = payload.get("labels", [])
            if not labels:
                return "Classification returned no results."
            lines = [f"  {i+1}. {l['label']} ({l['confidence']:.0%})" for i, l in enumerate(labels)]
            return "Image classification:\n" + "\n".join(lines)
        if name == "detect_clothing":
            image_url = str(args["image_url"]).strip() if args.get("image_url") else None
            image_b64 = str(args["image_base64"]).strip() if args.get("image_base64") else None
            image_file = str(args["image_file"]).strip() if args.get("image_file") else None
            if not image_url and not image_b64 and not image_file:
                return "detect_clothing: provide image_url, image_base64, or image_file"
            try:
                conf = float(args.get("confidence_threshold", 0.3))
            except (TypeError, ValueError):
                conf = 0.3
            payload = await self.tools.run_detect_clothing(
                image_url, image_b64, image_file, conf,
                self.state.approval, self._confirm_tool,
            )
            if "error" in payload:
                return f"detect_clothing failed: {payload['error']}"
            items = payload.get("items", [])
            if not items:
                return "No clothing items detected."
            lines = [f"  \u2022 {item['category']} ({item['confidence']:.0%})" for item in items]
            return f"Clothing detected ({len(items)} items):\n" + "\n".join(lines)
        if name == "embed_image":
            image_url = str(args["image_url"]).strip() if args.get("image_url") else None
            image_b64 = str(args["image_base64"]).strip() if args.get("image_base64") else None
            image_file = str(args["image_file"]).strip() if args.get("image_file") else None
            if not image_url and not image_b64 and not image_file:
                return "embed_image: provide image_url, image_base64, or image_file"
            normalize = bool(args.get("normalize", True))
            payload = await self.tools.run_embed_image(
                image_url, image_b64, image_file, normalize,
                self.state.approval, self._confirm_tool,
            )
            if "error" in payload:
                return f"embed_image failed: {payload['error']}"
            embedding = payload.get("embedding", [])
            dim = payload.get("dimension", len(embedding))
            model = payload.get("model", "clip_vit_b32")
            snippet = str(embedding[:4])[:-1] + ", ...]" if len(embedding) > 4 else str(embedding)
            return f"Image embedding ({dim}-dim, model={model}): {snippet}"
        if name == "analyze_image":
            image_url = str(args["image_url"]).strip() if args.get("image_url") else None
            image_b64 = str(args["image_base64"]).strip() if args.get("image_base64") else None
            image_file = str(args["image_file"]).strip() if args.get("image_file") else None
            if not image_url and not image_b64 and not image_file:
                return "analyze_image: provide image_url, image_base64, or image_file"
            payload = await self.tools.run_analyze_image(
                image_url, image_b64, image_file,
                bool(args.get("include_objects", True)),
                bool(args.get("include_labels", True)),
                bool(args.get("include_embedding", False)),
                float(args.get("confidence_threshold", 0.25)),
                int(args.get("top_k", 5)),
                self.state.approval, self._confirm_tool,
            )
            if "error" in payload:
                return f"analyze_image failed: {payload['error']}"
            parts = []
            objects = payload.get("objects") or []
            if objects:
                from collections import Counter as _C
                lc = _C(o["label"] for o in objects)
                parts.append("Objects: " + ", ".join(f"{v}\u00d7 {k}" for k, v in lc.most_common(5)))
            labels = payload.get("labels") or []
            if labels:
                top = labels[0]
                parts.append(f"Classification: {top['label']} ({top['confidence']:.0%})")
            if payload.get("embedding"):
                parts.append(f"Embedding: {payload['dimension'] or 512}-dim vector included")
            ms = payload.get("inference_ms", 0)
            return "\n".join(parts) + (f"\n(inference: {ms:.0f}ms)" if ms else "") if parts else "No analysis results."
    ...
    def _build_system_prompt(self) -> str:
        ...
        orchestration = (
            " TOOL ORCHESTRATION: For any task requiring more than one tool call"
            " or where the right sequence is unclear, call plan_task first with a clear task"
            " description and context. Read its STEPS output and execute each step in order,"
            " passing prior outputs forward. For simple single-tool tasks call the tool directly."
            " Use style='anime' in face_recognize for illustrated or anime-style images."
        )
    ...
    async def _load_default_model(self) -> None:
        ...
        if self.state.model not in models:
            # Prefer models already loaded in LM Studio to avoid evicting
            # models that other clients (e.g. dartboard) are actively using.
            chosen = None
            try:
                loaded = await self.client.list_loaded_models()
                if loaded:
                    chosen = loaded[0]
                    self._log_session(f"Using already-loaded model: {chosen}")
            except Exception:
                pass
            if chosen is None:
                chosen = models[0]
